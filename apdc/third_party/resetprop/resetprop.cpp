...
void load_prop_file(const char *filename, bool prop_svc) {
    ...
    parse_prop_file(filename, [=](auto key, auto val) -> bool {
        // parse_prop_file hands us string_views into the unmodified line buffer;
        // .data() is NOT NUL-terminated at the substring boundary, so passing it
        // directly to setprop sees "key=value" as one C string and rejects every
        // line via check_legal_property_name. Materialize null-terminated copies.
        std::string k(key), v(val);
        impl->setprop(k.c_str(), v.c_str(), prop_svc);
        return true;
    });
}
