...
// Insert or update a persistent property in /data/property.
// Mirrors persist_deleteprop's PB encode-and-rename flow; for the file mode
// (use_pb==false) writes /data/property/<name> directly.
bool persist_setprop(const char *name, const char *value) {
    if (use_pb) {
        ...
        bool ok = xwrite(fd, value, n) == static_cast<ssize_t>(n);
        close(fd);
        return ok;
    }
}
